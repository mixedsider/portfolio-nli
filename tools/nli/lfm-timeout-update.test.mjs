...
test("issue 6 defaults and probe preserve tokens, reasoning OFF and receive timeout", () => {
  ...
  assert.deepEqual([PROBE_ENDPOINTS.lfm.timeoutMs, PROBE_ENDPOINTS.qwen.timeoutMs], [6000, 16000]);
...
test("application clock stays open at 21s, rejects exactly 23s, and preserves smaller overrides", () => {
  for (const [configured, expected] of [[undefined, 23000], [99000, 23000], [7000, 7000]]) {
    ...
    try {
      ...
      if (expected === 23000) { time = 21000; deadline.check(); }
      time = expected - 1;
      deadline.check();
      time++;
...
test("cascade LFM caps at 6s and honors smaller explicit overrides", async () => {
  for (const [configured, expected] of [[99000, 6000], [6000, 6000], [1234, 1234]]) {
    const h = harness();
...
test("transport caps, cancellation and ignored late fetch release all permits", async () => {
  for (const [endpoint, field, cap] of [["lfm", "lfm", 6000], ["qwen", "model", 16000]]) {
    for (const configured of [99000, 1234]) {
...
test("real Gateway HTTP adopts a 4.5s delayed LFM within its stage budget, without Qwen", async (t) => {
  let calls = 0;
  ...
  // Adoption enforces the 6s stage cap; HTTP round-trip time also includes Gateway preparation.
  assert.ok(elapsedMs > 4000, `real elapsed ${elapsedMs}ms`);
...
test("Gateway fake clock accepts Qwen beyond 21s but rejects responses at and beyond 23s", async (t) => {
  for (const completedAt of [21400, 23000, 23001]) {
    let time = 0;
    const calls = [];
    const events = [];
    const gateway = await createNliServer({ config: createGatewayConfig({}), context, now: () => time,
      observer: (event) => events.push(event),
      lfmClient: async () => { calls.push("lfm"); time = 5500; return inspected("lfm", partial()); },
      verifier: { verify: async (options) => {
        calls.push("metadata"); assert.equal(options.deadlineAt, 21500); time = 6400;
        return { ok: true, returnedModelId: "fixture-qwen" };
      }, invalidate() {} },
    ...
    await response.json();
    assert.equal(response.status, completedAt === 21400 ? 200 : 503);
    assert.deepEqual(calls, ["lfm", "metadata", "qwen"]);
    assert.equal(events.some((event) => event.type === "complete" && event.stage === "qwen"), completedAt === 21400);
